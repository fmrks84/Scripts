...
pd.cd_classe||' - '||cl.ds_classe classe,
pd.cd_sub_cla||' - '||slc.ds_sub_cla sub_classe,
isp.cd_simpro,
pf.cd_pro_fat||' - '||pf.ds_pro_fat procedimento,
ts.cd_tip_tuss||' - '||tp.ds_tip_tuss tab_tuss,
ts.cd_tuss||' - '||ts.ds_tuss tuss,
ts.cd_convenio||' - '||conv.nm_convenio convenio,
...
inner join dbamv.tab_Fat tf on tf.cd_tab_fat = vp.cd_tab_fat and tf.ds_tab_fat like 'HSC%'
...
--and pf.cd_gru_pro in (71, 7, 12, 92, 15, 43, 44, 94)
--and pf.cd_gru_pro in (9, 71, 7, 8, 12, 91, 92, 15, 89, 43, 44, 94, 95, 96)
and pf.cd_gru_pro in (8,9,89,91,95,96)
