select xp.tp_opme_vl_referencia from empresa_convenio xp where xp.cd_convenio in 
 (select xx.cd_convenio  from cirurgia_aviso xx where xx.cd_aviso_cirurgia = 403699);


select * from pro_fat where cd_pro_fat in('00291711')--,);
select * from gru_pro where cd_gru_pro in (89,95,96,9)
select * from val_pro where cd_tab_fat in (2) and cd_pro_fat in ('00291711') order by 3 desc 
select * from produto where cd_pro_fat in ('090139850')--,09011140)
SELECT * FROM TAB_FAT WHERE CD_TAB_fAT = 93

select  xx.cd_aviso_cirurgia,xx.cd_convenio,xx.cd_con_pla
from cirurgia_aviso xx where xx.cd_aviso_cirurgia in (398161)
