select
  ...
  cd_prestador,
  dias_internacao,
  nvl(sum(quantidade_diarias_solicitada),0)total_diaria_solicitados,
  nvl(sum(quantidade_diarias_autorizados),0)total_diaria_autorizados,
  (nvl(sum(quantidade_diarias_solicitada),0) - nvl(sum(quantidade_diarias_autorizados),0))diarias_pendentes_autorizacao
from 
(
select
a.cd_guia cd_guia,
...
case when pf.cd_gru_pro = 1 then a.nr_dias_solicitados end quantidade_diarias_solicitada,--teste_diarias_solicitadas,
case when pf.cd_gru_pro = 1 then a.nr_dias_autorizados end quantidade_diarias_autorizados,--teste_diarias_autorizados,
--a.nr_dias_solicitados quantidade_diarias_solicitada,
--a.nr_dias_autorizados quantidade_diarias_autorizados, --- solicitado inclusão 14-09-2021
case when upper(h.ds_tip_acom)  like 'ENF%'
     then 'ENFERMARIA'
     when h.ds_tip_acom  like 'UTI%'
     then 'UTI'
     when h.ds_tip_acom  like 'SEMI INTENSIVA%'
     then 'SEMI_INTENSIVA'
...
b.qt_autorizada_convenio quantidade_autorizada, -- incluido 14-09-2021
trunc(b.dt_geracao) dh_solicitado,
trunc(sysdate - trunc(d.dt_atendimento))dias_internacao
from
dbamv.guia a
...
where a.cd_atendimento = 3261910--2639915
and pf.cd_gru_pro = 1
