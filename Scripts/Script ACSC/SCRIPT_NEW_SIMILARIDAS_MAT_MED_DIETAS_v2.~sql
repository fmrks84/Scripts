...
and gp.cd_gru_pro  in (7, 8, 9, 12, 15, 43, 44, 71, 89, 92, 94, 95, 96)
...
and ts.cd_multi_empresa in (25) -- coloar a codigo da empresa 
and ts.cd_convenio IN (205,206)
...
inner join dbamv.tab_FAt tb on tb.cd_tab_fat = vp.cd_tab_fat and tb.ds_tab_fat like '%HCNSC%' 
...
--and (ts.cd_convenio  in (205,206) or ts.cd_convenio IS NULL) -- aqui você colocar se irá trazer aberto quando colocar 'is null' ou se colocar o codigo do convenio deve alterar para in () para que traga fechado
order by pd.cd_produto

--select * from gru_pro gp where (gp.ds_gru_pro like '%MATERIAL%' OR GP.DS_GRU_PRO LIKE '%MEDICAM%' OR GP.DS_GRU_PRO LIKE '%ORTESE%')
