...
(5809929)--(4905377,4905636,4906190,4914786,4914193,4910159,4911160)--Empresa 4
order by 2
...
where it.cd_atendimento in (5809929)--(4905377,4905636,4906190,4914786,4914193,4910159)
...
select * from setor where  cd_multi_empresa = 3 order by 2 --and --cd_Setor in (3729,1043,1074,3727) order by 2
select * from setor where  cd_Setor in (3733) order by 2


--- C2307/2859 / C2307/2727
/*update dbamv.protocolo_doc x set x.Cd_Setor_Destino = 3734 where x.cd_setor_destino = 2390 and x.cd_protocolo_doc = 3474132;
commit;*/
update dbamv.protocolo_doc x set x.cd_setor = 3735  where x.cd_setor = 3714 and x.cd_protocolo_doc = 3886139
--;
--commit;
update dbamv.protocolo_doc x set x.Cd_Setor_Destino = 941 where x.cd_setor_destino = 3734 and x.cd_protocolo_doc = 4038174--;
--commit;
...
select * from fatura where cd_convenio = 742  order by cd_fatura desc ;
select * from remessa_fatura where cd_fatura = 51194
