...
SELECT  E.cd_multi_empresa cd_multi_empresa,
 ...
 WHERE 1 = 1
   AND A.CD_CONVENIO in (7,8,41,3,59,23,38,39,50,11,12,32,48,42,13,16,22,33,641) --RETIRADO CONV. 5 AMIL - CHAMADO C2212/7521
...
SELECT  E.cd_multi_empresa cd_multi_empresa,
  ...
  FROM IT_AGENDA_CENTRAL A,
       ...
       IT_MOVIMENTO_AGENDA_CENTRAL I
 WHERE 1 = 1
   AND A.CD_CONVENIO in (8,3,23,38,39,50,11,12,32,42,13,16) --retirado conv. 48 - chamado C2203/12518,
   --retirado conv 59 - C2307/10387 ,
   --retirado conv 5- C2308/11864
   --retirado conv (41,33,22)- C2309/7081
   AND A.cd_item_agendamento = B.cd_item_agendamento
   AND B.cd_exa_rx = C.cd_exa_rx
   AND C.cd_modalidade_exame = D.cd_modalidade_exame
   AND F.Cd_Movimento_Agenda_Central = I.CD_MOVIMENTO_AGENDA_CENTRAL
...
SELECT  E.cd_multi_empresa cd_multi_empresa,
  ...
  FROM IT_AGENDA_CENTRAL A,
       ...
       IT_MOVIMENTO_AGENDA_CENTRAL I
 WHERE 1 = 1
   AND A.CD_CONVENIO in (8,3,23,38,39,50,11,12,32,42,13,16)
   --retirado conv. 48 - chamado C2203/12518 ,
   --reitrado conv.59 C2307/10387,
   --retirado conv.5 C2308/11864
   --retirdar conv.41,33,22 - C2309/7081
   AND A.cd_item_agendamento = B.cd_item_agendamento
   AND B.cd_exa_rx = C.cd_exa_rx
   AND C.cd_modalidade_exame = D.cd_modalidade_exame
   AND F.Cd_Movimento_Agenda_Central = I.CD_MOVIMENTO_AGENDA_CENTRAL
...
SELECT  E.cd_multi_empresa cd_multi_empresa,
  ...
  FROM IT_AGENDA_CENTRAL A,
       ...
       IT_MOVIMENTO_AGENDA_CENTRAL I
 WHERE 1 = 1
   AND A.CD_CONVENIO in (7,8,41,5,3,59,23,38,39,50,11,12,32,48,42,13,16,22,33,641)
   AND A.cd_item_agendamento = B.cd_item_agendamento
   AND B.cd_exa_rx = C.cd_exa_rx
   AND C.cd_modalidade_exame = D.cd_modalidade_exame
   AND F.Cd_Movimento_Agenda_Central = I.CD_MOVIMENTO_AGENDA_CENTRAL
...
WHERE
  ...
  AND A.CD_CONVENIO IN (7, 8, 5, 41, 59, 23, 35, 38, 39, 9, 50, 11, 12, 32, 48, 42, 13, 16, 22, 33,641)
...
SELECT  E.cd_multi_empresa cd_multi_empresa,
 ...
 WHERE 1 = 1
   ...
   --retirado conv. 59 C2307/10387,
   --retirado conv.5 C2308/11864
   --retirado conv 41,33,22 C2308/7081
   AND A.cd_item_agendamento = B.cd_item_agendamento
   AND B.cd_exa_rx = C.cd_exa_rx
   AND C.cd_modalidade_exame = D.cd_modalidade_exame
   AND F.Cd_Movimento_Agenda_Central = I.CD_MOVIMENTO_AGENDA_CENTRAL
...
SELECT  E.cd_multi_empresa cd_multi_empresa,
  ...
  FROM IT_AGENDA_CENTRAL A,
       ...
       IT_MOVIMENTO_AGENDA_CENTRAL I
 WHERE 1 = 1
   AND A.CD_CONVENIO in (7,8,3,23,38,39,50,11,12,32,42,13,16,641)
    --retirado conv. 48 - chamado C2203/12518 ,
    -- retirado conv. 59 - chamado C2307/10387,
    -- retirado conv.5 C2308/11864
    -- retirado conv 41,33,22
   AND A.cd_item_agendamento = B.cd_item_agendamento
   AND B.cd_exa_rx = C.cd_exa_rx
   AND C.cd_modalidade_exame = D.cd_modalidade_exame
   AND F.Cd_Movimento_Agenda_Central = I.CD_MOVIMENTO_AGENDA_CENTRAL
...
UNION ALL
----------------------------------------------------------------------------------------
----- 03 - CASA DE SAUDE SAO JOSE
----------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------
----- BRADESCO
----------------------------------------------------------------------------------------
 SELECT E.CD_MULTI_EMPRESA CD_MULTI_EMPRESA,
        (TO_CHAR(F.CD_MOVIMENTO_AGENDA_CENTRAL)||'.'||CASE WHEN C.EXA_RX_CD_PRO_FAT IN ('40901106','40901050','40901084','20202016','40701131','40701140','40701069','40701034','40701050')  THEN '2' ELSE '1' END||'-'||TO_CHAR(A.HR_AGENDA,'ddmm')) ID,
        A.CD_CONVENIO,
        G.NR_REGISTRO_OPERADORA_ANS,
        F.NM_PACIENTE,
        (SELECT DS_RESPOSTA FROM PER_PEDRX WHERE CD_IT_AGENDA_CENTRAL = A.CD_IT_AGENDA_CENTRAL AND CD_PERGUNTA = 102 AND ROWNUM = 1) NR_CARTEIRA,
...
WHERE nM_paciente like '%TICIANA%KATSU'
---AND cd_convenio = 73 
--AND trunc(DH_SOLICITADO) > = '01/10/2023'
--ID = '508584.1-1704'
GROUP BY CD_MULTI_EMPRESA,
    ...
    QT_DIARIAS_SOLICITADA,
    TP_ATENDIMENTO
ORDER BY DH_SOLICITADO DESC 

SELECT * FROM DBAMV.VW_OVMD_GUIA_SOLICITADAS

SELECT ATD.CD_ATENDIMENTO, ATD.TP_ATENDIMENTO,PCT.* FROM ATENDIME ATD 
INNER JOIN PACIENTE PCT ON PCT.CD_PACIENTE = ATD.CD_PACIENTE
WHERE ATD.CD_CONVENIO = 641 ORDER BY 1 DESC 
