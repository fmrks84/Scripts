select c.nm_cor||' - '||e.ds_especialid||' - '|| replace (replace (translate (initcap (T.NM_PACIENTE),'abcdefghijklmnopqrstuvxzywç','¬'),' ','.'),'¬','') || DECODE (T.NM_PACIENTE,NULL,NULL,'.')observacao from sacr_cor_referencia c, especialid e, triagem_atendimento t where c.cd_cor_referencia = t.cd_cor_referencia and  e.cd_especialid = t.cd_especialid and t.cd_atendimento = 5847723
;
