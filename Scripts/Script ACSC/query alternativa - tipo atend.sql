...
----------------------
--C2405/6076 -- query alternativa guia sadt campo nr_guia_operadora , senha , dt_autorizacao para ajuste do problema de não inserir a guia de atendimento 
--na tela de reg_amb
select * 
from atendime 
where cd_convenio = 48
and cd_pro_int = '10000414'
order by 7 desc 

select distinct min(g.nr_guia) from dbamv.guia g where g.cd_atendimento = :par2 and g.tp_guia = 'P'
select distinct min(g.Cd_Senha) from dbamv.guia g where g.cd_atendimento = :par2 and g.tp_guia = 'P' 
select distinct min(to_char(g.dt_autorizacao,'rrrr-mm-dd'))  from dbamv.guia g where g.cd_atendimento = :par2 and g.tp_guia = 'P'

select * from reg_Fat where cd_remessa = 422921
select /*xp.cd_remessa,
       sum(xp.vl_itfat_nf)*/
       *
 from itfat_nota_fiscal xp 
 where xp.cd_reg_fat in (select cd_Reg_Fat from reg_Fat where cd_remessa = 422921)
 group by xp.cd_remessa
 
 select * from con_Rec where cd_reg_Fat = 621462
 select * from 
