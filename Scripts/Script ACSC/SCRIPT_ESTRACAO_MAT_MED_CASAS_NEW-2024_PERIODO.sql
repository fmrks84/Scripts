...
--and conv.cd_convenio IN (7,641,32)
and irg.cd_gru_pro in (7, 12, 15, 43, 44, 55,71, 92, 94)
...
--x.ds_produto,
/*xx.cd_pro_fat cd_pro_Fat_mestre,
xx.ds_pro_fat ,*/
ts.cd_tip_tuss,
ts.cd_tuss,
ts.ds_tuss,
...
and ts.cd_tip_tuss in (19,20,00) and ts.cd_convenio is null--and (ts.cd_convenio  in (107) or ts.cd_convenio IS NULL) -- IN (7,641,32)
...
inner join tab_Fat tf on tf.cd_tab_fat = vp.cd_tab_fat and tf.ds_tab_fat like '%CSSJ%'
...
and pd.dt_ultima_entrada between '01/02/2023' and '01/02/2024'
and tft.cd_gru_pro in (7, 12, 15, 43, 44, 55,71, 92, 94)
order by 1

--select * from val_pro vp where vp.cd_tab_fat in (581) and vp.cd_pro_fat = '90000382'
--select * from gru_pro where ds_gru_pro like 'DIETAS%'
--select * from tab_Fat where cd_tab_fat in (1, 181, 526, 581, 1081, 2868)
--select * from gru_pro where cd_gru_pro in (7, 12, 15, 43, 44, 55,71, 92, 94)
/*
