...
and irg.cd_gru_pro in (8, 9, 57, 66, 89, 91, 95, 96)
...
and ts.cd_tip_tuss in (19,20,00)  --and (ts.cd_convenio  in (187,207) or ts.cd_convenio IS NULL) -- IN (7,641,32)
--aND TS.CD_MULTI_EMPRESA = 25 
AND TS.CD_CONVENIO Is null --(187,287)
...
and tft.cd_gru_pro in (8, 9, 57, 66, 89, 91, 95, 96)
order by 1
...
select * from gru_pro where (ds_gru_pro like '%MAT%' OR ds_gru_pro like '%ORTES%')
